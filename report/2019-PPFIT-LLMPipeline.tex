\documentclass{PPFIT} % English
\usepackage{tikz}
\usetikzlibrary{positioning}

\hypersetup{
	pdftitle={LLM-Based Translation Pipeline for Functional Coverage Analysis},
	pdfauthor={Pavel Lukl},
	pdfkeywords={DO-178C, LLM, requirement formalization, agentic pipeline, functional coverage}
}

\PPFinalCopy

\PPYear{2025/2026}

\PaperTitle{LLM-Based Translation Pipeline for Functional Coverage Analysis}

\Authors{Pavel Lukl*}
\affiliation{*%
  \href{mailto:xluklp00@stud.fit.vut.cz}{xluklp00@stud.fit.vut.cz},
  \textit{Faculty of Information Technology, Brno University of Technology}}

\Keywords{DO-178C --- LLM --- requirement formalization --- agentic pipeline --- functional coverage}

%\Supplementary{}

\Abstract{
Safety-critical avionics software requires formal coverage analysis of requirements, but requirements are written in natural language. Manual formalization is slow, error-prone, and does not scale. This report investigates the use of large language models (LLMs) in a multi-step agentic pipeline to automatically translate natural-language requirements into a domain-specific predicate formalism. We survey existing approaches to requirement formalization, design a four-step pipeline (ambiguity review, flavour extraction, entity extraction, and constraint formalization with structural validation), and conduct preliminary experiments comparing orchestration frameworks, output representations (JSON vs Python), task decomposition strategies (multi-call pipeline vs single-call chain-of-thought), and local vs cloud models. Our findings show that model capability is the strongest predictor of output quality, that JSON outperforms Python on smaller local models despite the opposite being reported for frontier models in the literature, and that structured decomposition in prompts improves results regardless of architecture. These preliminary results establish a foundation for the next phase of the project, which will focus on comprehensive evaluation, human-in-the-loop review, and iterative improvement through fine-tuning.
}

\Teaser{
%	\TeaserImage{placeholder.pdf}
}

\begin{document}
\startdocument

\input{2019-PPFIT-LLMPipeline-text-en.tex}

\phantomsection
\ifenglish
    \bibliographystyle{bib-styles/enplain}
\fi

\bibliography{2019-PPFIT-LLMPipeline-bib}

\newpage
\onecolumn
\appendix
\input{2019-PPFIT-LLMPipeline-appendix-en.tex}

\end{document}
