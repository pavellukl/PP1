...
\begin{frame}
  ...
  \bigskip\bigskip
  \textbf{3.}\quad Semantic correctness is the real challenge\\
...
\begin{frame}
  ...
  JSON performed better on phi4:14b (local).
...
\begin{frame}
  ...
  \begin{quote}\itshape
  A complex avionics requirement involving a timed signal toggle
  with multiple conditions: peak value comparison, event counting
  after a detection event, and value comparison with a previous state.
  \end{quote}
  ...
  \ttfamily\quad\colorbox{green!20}{AllOf(} \\
  \ttfamily\quad\quad\colorbox{green!20}{Happening(ev\_write),} &
  \ttfamily\quad\quad\colorbox{green!20}{Happening(ev\_write),} \\
  \ttfamily\quad\quad\colorbox{green!20}{Val(x, \textit{at max peak})} &
  \ttfamily\quad\quad\colorbox{yellow!30}{Val(max\_peak)} \\
  \ttfamily\quad\quad\colorbox{green!20}{\;- Val(x, \textit{at min peak}) > T,} &
  \ttfamily\quad\quad\colorbox{yellow!30}{\;- Val(min\_peak) > T,} \\
  \ttfamily\quad\quad\colorbox{green!20}{EvtOccCount = 2,} &
  \ttfamily\quad\quad\colorbox{green!20}{EvtOccCount = 2,} \\
  \ttfamily\quad\quad\colorbox{red!20}{Val(x) > Val(x, \textit{prev write})} &
  \ttfamily\quad\quad\colorbox{red!20}{Val(x) > ValBefore(x)} \\
  \ttfamily\quad ),  &
  \ttfamily\quad ), \\
  \ttfamily\quad\colorbox{green!20}{Val(y) = $\neg$ValBefore(y),} &
  \ttfamily\quad\colorbox{green!20}{Val(y) != ValBefore(y),} \\
  \ttfamily\quad\colorbox{green!20}{duration} &
  \ttfamily\quad\colorbox{green!20}{duration} \\
  \ttfamily ) & \ttfamily ) \\
  \end{tabular}
  ...
  Peak lookup & \cellcolor{red!20}$\times$ & \cellcolor{yellow!30}$\sim$ & \cellcolor{green!20}\checkmark \\
  Toggle effect & \cellcolor{red!20}$\times$ & \cellcolor{green!20}\checkmark & \cellcolor{green!20}\checkmark \\
  Prev.\ value & \cellcolor{red!20}$\times$ & \cellcolor{red!20}$\times$ & \cellcolor{red!20}$\times$ \\
  \end{tabular}
\end{frame}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \bigskip
    \item \textbf{Semantic correctness} is the harder problem\\
    {\small (structural validity is largely solved)}
  \end{enumerate}
